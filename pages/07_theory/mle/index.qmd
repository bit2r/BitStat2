...
server <- function(input, output) {
  ...
  output$norm_plot <- renderPlot({
    ggplot(data.frame(x = norm_data()), aes(x)) +
      geom_histogram(aes(y = ..density..), bins = 30, fill = "#c9c6b8", color = "white") +
      stat_function(fun = dnorm, args = list(mean = norm_estimates()$mean, sd = norm_estimates()$sd),
                    color = "#8a1500", size = 1.5) +
      geom_vline(xintercept = norm_estimates()$mean, linetype = "dashed", color = "#4a4a44", size = 1) +
      geom_errorbarh(aes(xmin = norm_estimates()$mean - norm_estimates()$se_mean,
                         xmax = norm_estimates()$mean + norm_estimates()$se_mean,
                         y = 0), color = "#4a4a44", height = 0.01, size = 1) +
  ...
  output$pois_plot <- renderPlot({
    ggplot(data.frame(x = pois_data()), aes(x, fill = "#c9c6b8")) +
      geom_histogram(aes(y = ..count.. / sum(..count..)), bins = 30, color = "white") +
      geom_vline(xintercept = pois_estimates()$lambda, linetype = "dashed", color = "#4a4a44", size = 1) +
  ...
  output$binom_plot <- renderPlot({
    ggplot(data.frame(x = binom_data()), aes(x)) +
      geom_histogram(aes(y = ..count.. / sum(..count..)), bins = min(30, max(binom_data())), fill = "#c9c6b8", color = "white") +
      geom_vline(xintercept = binom_estimates()$prob * input$binom_size, linetype = "dashed", color = "#4a4a44", size = 1) +
